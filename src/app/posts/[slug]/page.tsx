...
export default async function Post({
  ...
  const { PostContent, metadata, raw, toc } = current;
  const { minutes } = readingTime(raw);

  const slugger = new GithubSlugger();

  const catalog = flattenTree(toc as any[], 'children').map((node) => ({
  ...
  return (
    <TOCProvider toc={catalog}>
      <div className="relative">
        {catalog.length ? (
          <aside className="absolute inset-y-0 left-full ml-6 hidden xl:block">
            <FadeIn className="fixed" order={1}>
              <PageTOC className="pr-4 max-h-[50svh] max-w-3xs truncate">
                <PageTOCItems />
              </PageTOC>
            </FadeIn>
          </aside>
        ...
        <div className="space-y-12">
          <div className="space-y-6">
            ...
            <FadeIn order={4}>
              <div className="prose prose-headings:scroll-m-28 dark:prose-invert">
                <PostContent />
              </div>
            </FadeIn>
          </div>
