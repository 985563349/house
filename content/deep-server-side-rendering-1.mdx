...
description: "用 React、Express 和 Vite 搭建同构渲染，串起水合、路由与数据获取。"
categories:
  - React
  - SSR
---
...
export default getPageRoutes(
  import.meta.glob('/src/pages/**/page.jsx', { eager: true }),
);
```
        ...
        {routes.map((route) => {
          ...
          return (
            <Route
              ...
              path={path}
              {/* !diff + */}
              element={<RouteElement ctx={ctx} {...route} />}
            />
          );
        })}
      </Routes>
...
hydrateRoot(
  ...
  // !diff +
  <Suspense>{createRouter({ routes })}</Suspense>,
);
```
...
export async function render({ ctx, url }) {
  ...
  const html = renderToString(
    <Suspense>{createRouter({ ctx, routes, url })}</Suspense>,
  );
  // !diff(end)
