...
ThreeJS 中使用环境贴图首先需要选择适合的加载器加载贴图，然后将 **scene.background** 属性设置为贴图实例，最后把需要接收光照的物体的材质属性 **envMap** 设置为贴图实例。

> TIP: 通过将 **scene.environment** 属性设置为环境贴图实例，能快速为场景内所有物理材质添加环境贴图。
