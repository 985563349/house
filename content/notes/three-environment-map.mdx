...
在真实世界里，一个物体，不会单独存在于一个空空的环境中，它的周围一定有其他的物体。当光源照射到其他物体上的时候，一定会产生反射，其中就会有很多反射的光线会反射到该物体上。3D世界中，如果一个物体能接受来自周围环境的光照，所呈现出来的效果就会更加真实。

基于图像的光照就是将物体周围的光照信息存储在图片（即环境贴图）中，3D引擎能够加载并识别这些信息，从而模拟出非常真实的光照效果。

图像光照技术通常会和物理材质一起使用，非物理材质在底层实现上无法接受来自周围环境的光照。
...
ThreeJS 中使用环境贴图首先需要选择适合的加载器加载贴图，然后将 **scene.background** 属性设置为贴图实例，最后把需要接受光照的物体的材质属性 **envMap** 设置为贴图实例。
