...
嗯？这听起来有点像是同构渲染，但请相信我，它们之间有着非常大的区别。在后续的文章中，我会再详细地去介绍。
...
因为直觉告诉我，在 React 中函数组件不能是异步的。并且一直以来 React 也不允许在渲染过程中直接进行副作用操作。

现在这样做岂不是完全背离了 React 的原则？

而事实上，并非如此。

原因是因为，在 React 的生命周期内，**服务器组件不会重新渲染，它们只在服务器上运行一次**。
...
服务队组件可以缓存运行结果，这些结果可以在后续请求中重用，并且跨不同客户端共享。
