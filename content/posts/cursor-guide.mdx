...
![auto-renewal](/cursor-auto-renewal.gif 'auto-renewal')

Cursor 能在你编写代码时理解你的意图，并通过强大的自动补全功能预测你的下一步代码并提供智能建议，就像有一位程序高手在与你结对编程。

### 多行编辑

![multiline-editing](/cursor-multiline-editing.gif 'multiline-editing')

Cursor 可以同时建议多行编辑，这不仅是为了节省键击次数，更是为了让你保持专注。

### 智能重写

![smart-rewrites](/cursor-smart-rewrites.gif 'smart-rewrites')

我们都有手指跟不上大脑的时候，打字错误会经常发生。Cursor 能及时发现并修复你的错误，就像一个智能拼写检查器。

### 光标预测

![cursor-prediction](/cursor-cursor-prediction.gif 'cursor-prediction')
...
如果 Tab 是你的编程伙伴，那么 ⌘ K 就是你的编程向导。此快捷键能让你轻松掌握 AI 辅助编程的全部功能。

### 按需生成代码

![code-generation](/cursor-code-generation.gif 'code-generation')

编写复杂的函数，不知道从哪里开始？⌘ K 可以帮你。你只需要描述出想要编写的功能，然后 Cursor 就能帮你快速生成代码。

### 快速重构代码

![refactor-code](/cursor-refactor-code.gif 'refactor-code')

还记得上次重构代码的时候吗？你可能花了不少时间。现在，使用 ⌘ K，就能轻松搞定！你只需要选中代码，按下快捷键，然后描述修改需求，剩下的就交给它处理。

### 代码解读

![code-reading](/cursor-code-reading.gif 'code-reading')
...
![terminal](/cursor-terminal.gif 'terminal')
...
![models](/cursor-setting-models.gif 'models')

使用 ⌘ L 快捷键就能打开 Cursor 的聊天对话面板，在其中你可以根据需求灵活切换不同模型。

![chat](/cursor-chat.gif 'chat')
...
![context](/cursor-context.gif 'context')

### 一键应用代码

在聊天窗口中看到满意的建议？只需一键，就能直接将其添加到你的代码中，无需繁琐的复制粘贴。

![chat-apply](/cursor-chat-apply.gif 'chat-apply')

### 图像识别

有时你可能难以用语言完整描述需求，别担心！使用 Cursor 的聊天功能时，你可以直接将图像加入对话，它能准确的理解并响应你的意图。

![image-recognition](/cursor-image-recognition.gif 'image-recognition')
...
![import-web](/cursor-import-web.gif 'import-web')

或者，让它 review 代码提交记录。

![commit-review](/cursor-commit-review.gif 'commit-review')

甚至，直接从网络上搜索信息。

![chat-web](/cursor-chat-web.gif 'chat-web')
...
![context-docs](/cursor-context-docs.gif 'context-docs')

此外，Cursor 还支持添加自定义文档，它能自动提取链接中的内容并纳入索引。我们可以在 Cursor Setting 中找到添加文档的选项。

添加文档时需要注意，若想索引整个站点的页面，则必须在 URL 中添加根路径进行访问。

![context-custom-docs](/cursor-context-custom-docs.gif 'context-custom-docs')
...
![context-create-notepad](/cursor-context-create-notepad.gif 'context-create-notepad')

之后与 AI 交互时，就可以使用 @Notepads 来引入笔记。

![context-use-notepad](/cursor-context-use-notepad.gif 'context-use-notepad')
...
![agent-generate-game](/cursor-agent-generate-game.gif 'agent-generate-game')
...
![agent-accept](/cursor-agent-accept.gif 'agent-accept')

对于已应用的代码更改，你可以点击 Restore checkpoint 进行撤销。撤销后，若需要恢复，可以点击 Redo checkpoint 重做。

![agent-checkpoint](/cursor-agent-checkpoint.gif 'agent-checkpoint')
...
![global-rules](/cursor-global-rules.gif 'global-rules')

也可以在项目中创建 .cursor/rules 目录，并通过 .mdc 文件添加基于项目的 Rules。之后在与 AI 交互时，Cursor 会自动加载该目录下的所有 Rule。

![project-rules](/cursor-project-rules.gif 'project-rules')
...
通过本文的介绍，相信你已经对 Cursor 的主要功能有了全面的了解。从智能的 Tab 补全到强大的 ⌘ K 功能，从上下文感知的 Chat 到自主规划的 Agent，再到灵活的 Rules 配置，Cursor 为我们提供了一个强大而智能的编程环境。
